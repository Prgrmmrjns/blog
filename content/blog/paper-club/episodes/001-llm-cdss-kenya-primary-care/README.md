# Episode 1 — LLM CDSS in Kenyan primary care

| | |
| --- | --- |
| Paper | Agweyu et al., *Nature Medicine* (26 Jun 2026) |
| DOI | https://doi.org/10.1038/s41591-026-04503-6 |
| License | CC BY 4.0 |
| Runtime | ~11–13 min |
| Post | [`content/blog/en/ai-medicine-paper-club-kenya-llm-cdss.md`](../../../en/ai-medicine-paper-club-kenya-llm-cdss.md) |

## Files

- [`script.md`](script.md) — spoken script with slide cues
- [`slides.md`](slides.md) — Marp source
- [`slides.html`](slides.html) — rendered deck (open in a browser)
- [`slides.pdf`](slides.pdf) — 19-page PDF for recording or sharing
- [`figures/`](figures/) — study images + [`SOURCES.md`](figures/SOURCES.md)

Public copies for the live site: `/paper-club/episodes/001-llm-cdss-kenya-primary-care/`

## Record

Open `slides.pdf` or `slides.html`, full screen, read `script.md`. Advance on each `## SLIDE n` heading.
