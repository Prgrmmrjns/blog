...
const links = computed(() => [
  { to: "/freelancing", label: t("nav.freelancing"), hard: false },
  ...
  <nav class="fixed left-0 right-0 top-0 z-50 border-b border-gray-200 bg-[#f5f3eb]/90 backdrop-blur-md transition-colors dark:border-white/5 dark:bg-[#0e0e0e]/80">
    <div class="mx-auto flex h-16 max-w-6xl items-center gap-3 px-4 sm:px-6">
      <NuxtLink
        ...
        class="shrink-0 text-gray-900 transition-colors hover:text-teal dark:text-cream dark:hover:text-mint"
      >
        <SiteLogo />
      </NuxtLink>
      <div class="flex min-w-0 flex-1 items-center gap-3 overflow-x-auto sm:gap-6">
        <template v-for="link in links" :key="link.to">
          <a
            v-if="link.hard"
            :href="link.to"
            class="whitespace-nowrap text-xs font-medium text-gray-600 transition-colors hover:text-teal dark:text-cream/70 dark:hover:text-mint sm:text-sm"
