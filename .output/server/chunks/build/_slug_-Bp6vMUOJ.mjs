import process from 'node:process';globalThis._importMeta_=globalThis._importMeta_||{url:"file:///_entry.js",env:process.env};import { c as useRoute, d as createError, u as useHead, _ as __nuxt_component_0$1, a as useRuntimeConfig, b as useNuxtApp } from './server.mjs';
import { defineComponent, withAsyncContext, unref, mergeProps, withCtx, createVNode, createTextVNode, computed, toRaw, ref, resolveComponent, defineAsyncComponent, h, getCurrentInstance, reactive, watch, Text, Comment, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderAttr, ssrRenderComponent, ssrInterpolate, ssrRenderList, ssrRenderSlot, ssrIncludeBooleanAttr } from 'vue/server-renderer';
import { _ as _export_sfc } from './_plugin-vue_export-helper-1tPrXgE0.mjs';
import { D as pascalCase, E as kebabCase, f as destr } from '../nitro/nitro.mjs';
...
const __nuxt_component_1 = /* @__PURE__ */ Object.assign(_export_sfc(_sfc_main$4, [["ssrRender", _sfc_ssrRender]]), { __name: "IconArrowLeft" });
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<section${ssrRenderAttrs(mergeProps({ class: "mt-16 border-t border-gray-200 pt-16 dark:border-white/10" }, _attrs))}><h2 class="mb-6 text-2xl font-bold text-gold"> Comments </h2><form class="mb-14 rounded-2xl border border-gray-200 bg-gray-50 p-8 dark:border-white/10 dark:bg-[#161616]/50"><div class="space-y-6"><div><label for="author-name" class="mb-2 block text-sm font-medium text-gray-700 dark:text-cream/70"> Name (optional) </label><input id="author-name"${ssrRenderAttr("value", unref(authorName))} type="text" placeholder="Your name" class="w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-base text-gray-900 placeholder-gray-500 focus:border-teal focus:outline-none focus:ring-2 focus:ring-teal/30 dark:border-white/20 dark:bg-[#1a1a1a] dark:text-cream dark:placeholder-cream/40 dark:focus:border-mint/50 dark:focus:ring-mint/30"></div><div><label for="comment-content" class="mb-2 block text-sm font-medium text-gray-700 dark:text-cream/70"> Comment * </label><textarea id="comment-content" rows="5" required placeholder="Write a comment..." class="w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-base text-gray-900 placeholder-gray-500 focus:border-teal focus:outline-none focus:ring-2 focus:ring-teal/30 dark:border-white/20 dark:bg-[#1a1a1a] dark:text-cream dark:placeholder-cream/40 dark:focus:border-mint/50 dark:focus:ring-mint/30">${ssrInterpolate(unref(commentContent))}</textarea></div>`);
      if (unref(submitError)) {
        _push(`<p class="text-sm text-red-400">${ssrInterpolate(unref(submitError))}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<button type="submit"${ssrIncludeBooleanAttr(unref(submitting) || !unref(commentContent).trim()) ? " disabled" : ""} class="rounded-xl bg-teal/20 px-6 py-3 text-base font-semibold text-teal transition-colors hover:bg-teal/30 disabled:opacity-50 disabled:cursor-not-allowed dark:bg-mint/20 dark:text-mint dark:hover:bg-mint/30">${ssrInterpolate(unref(submitting) ? "Posting..." : "Post comment")}</button></div></form>`);
      if (unref(loading)) {
        _push(`<div class="text-gray-500 dark:text-cream/50"> Loading comments... </div>`);
      } else if (unref(error)) {
        _push(`<p class="text-red-400">${ssrInterpolate(unref(error))}</p>`);
      } else if (unref(comments).length === 0) {
        _push(`<div class="rounded-2xl border border-gray-200 bg-gray-50 px-12 py-16 text-center text-base text-gray-600 dark:border-white/10 dark:bg-[#161616]/50 dark:text-cream/50"> No comments yet. Be the first to comment! </div>`);
      } else {
        _push(`<ul class="space-y-8"><!--[-->`);
        ssrRenderList(unref(comments), (c) => {
          _push(`<li class="rounded-2xl border border-gray-200 bg-white p-8 shadow-md transition-shadow hover:shadow-lg dark:border-white/10 dark:bg-[#161616] dark:shadow-none dark:hover:border-white/20"><div class="mb-4 flex items-center gap-4"><div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-teal/20 text-lg font-bold text-teal dark:bg-mint/20 dark:text-mint">${ssrInterpolate((c.author_name || "A").charAt(0).toUpperCase())}</div><div class="flex flex-col gap-1"><span class="text-base font-semibold text-gray-900 dark:text-cream">${ssrInterpolate(c.author_name || "Anonymous")}</span><span class="text-sm text-gray-500 dark:text-cream/40">${ssrInterpolate(formatDate(c.created_at))}</span></div></div><p class="whitespace-pre-wrap text-base leading-relaxed text-gray-800 dark:text-cream/90">${ssrInterpolate(c.content)}</p></li>`);
        });
        _push(`<!--]--></ul>`);
      }
      _push(`</section>`);
...
_sfc_main$1.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/BlogComments.vue");
  return _sfc_setup$1 ? _sfc_setup$1(props, ctx) : void 0;
};
const __nuxt_component_3 = Object.assign(_sfc_main$1, { __name: "BlogComments" });
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      const _component_BlogComments = __nuxt_component_3;
      if (unref(post)) {
        _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen bg-[#f5f3eb] dark:bg-[#0e0e0e]" }, _attrs))}><div class="relative h-[40vh] min-h-[280px] w-full overflow-hidden">`);
        if (unref(post).image) {
          _push(`<img${ssrRenderAttr("src", unref(post).image)}${ssrRenderAttr("alt", unref(post).title)} class="h-full w-full object-cover">`);
        } else {
          _push(`<div class="flex h-full w-full items-center justify-center bg-teal/10 text-6xl text-teal/30"> # </div>`);
        }
        _push(`</div><div class="border-b border-gray-200 bg-[#f5f3eb] px-8 py-10 dark:border-white/5 dark:bg-[#0e0e0e] sm:px-12"><div class="mx-auto max-w-4xl">`);
        ...
        _push(`<h1 class="text-3xl font-extrabold leading-tight text-gray-900 dark:text-cream sm:text-5xl">${ssrInterpolate(unref(post).title)}</h1><div class="mt-4 flex flex-wrap items-center gap-4 text-gray-600 dark:text-cream/60"><span>${ssrInterpolate(unref(post).date)}</span><!--[-->`);
        ssrRenderList(unref(post).tags ?? [], (tag) => {
          _push(`<span class="rounded-full bg-gold/20 px-2.5 py-1 text-xs font-semibold text-gold">${ssrInterpolate(tag)}</span>`);
        });
        _push(`<!--]--></div></div></div><article class="mx-auto max-w-3xl px-8 py-16 sm:px-12"><div class="prose prose-gray-900 max-w-none dark:prose-invert dark:prose-cream">`);
        if (unref(post).body) {
        ...
        _push(`</div>`);
        _push(ssrRenderComponent(_component_BlogComments, { "post-slug": unref(slug) }, null, _parent));
        _push(`</article><footer class="border-t border-gray-200 py-12 text-center text-xs text-gray-500 dark:border-white/5 dark:text-cream/30"> Jonas Wolber · Curiosity is all you need. </footer></div>`);
      } else {
        _push(`<!---->`);
      }
    };
...
//# sourceMappingURL=_slug_-Bp6vMUOJ.mjs.map
