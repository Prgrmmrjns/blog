...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<section${ssrRenderAttrs(mergeProps({ class: "mt-16 border-t border-gray-200 pt-16 dark:border-white/10" }, _attrs))}><h2 class="mb-6 text-2xl font-bold text-gold"> Comments </h2><form class="mb-12 space-y-4"><div><label for="author-name" class="mb-2 block text-sm font-medium text-gray-700 dark:text-cream/70"> Name (optional) </label><input id="author-name"${ssrRenderAttr("value", unref(authorName))} type="text" placeholder="Your name" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 placeholder-gray-500 focus:border-teal focus:outline-none focus:ring-1 focus:ring-teal dark:border-white/20 dark:bg-[#161616] dark:text-cream dark:placeholder-cream/40 dark:focus:border-mint/50 dark:focus:ring-mint/50"></div><div><label for="comment-content" class="mb-2 block text-sm font-medium text-gray-700 dark:text-cream/70"> Comment * </label><textarea id="comment-content" rows="4" required placeholder="Write a comment..." class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 placeholder-gray-500 focus:border-teal focus:outline-none focus:ring-1 focus:ring-teal dark:border-white/20 dark:bg-[#161616] dark:text-cream dark:placeholder-cream/40 dark:focus:border-mint/50 dark:focus:ring-mint/50">${ssrInterpolate(unref(commentContent))}</textarea></div>`);
      if (unref(submitError)) {
        _push(`<p class="text-sm text-red-400">${ssrInterpolate(unref(submitError))}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<button type="submit"${ssrIncludeBooleanAttr(unref(submitting) || !unref(commentContent).trim()) ? " disabled" : ""} class="rounded-lg bg-mint/20 px-4 py-2 text-sm font-semibold text-mint transition-colors hover:bg-mint/30 disabled:opacity-50 disabled:cursor-not-allowed">${ssrInterpolate(unref(submitting) ? "Posting..." : "Post comment")}</button></form>`);
      if (unref(loading)) {
        ...
        _push(`<div class="rounded-lg border border-gray-200 bg-gray-50 p-6 text-center text-gray-600 dark:border-white/10 dark:bg-[#161616]/50 dark:text-cream/50"> No comments yet. Be the first to comment! </div>`);
      } else {
        _push(`<ul class="space-y-6"><!--[-->`);
        ssrRenderList(unref(comments), (c) => {
          _push(`<li class="rounded-lg border border-gray-200 bg-white p-6 shadow dark:border-white/10 dark:bg-[#161616]/50 dark:shadow-none"><div class="mb-2 flex items-center gap-3"><span class="font-semibold text-gray-900 dark:text-cream">${ssrInterpolate(c.author_name || "Anonymous")}</span><span class="text-xs text-gray-500 dark:text-cream/40">${ssrInterpolate(formatDate(c.created_at))}</span></div><p class="whitespace-pre-wrap text-[15px] leading-relaxed text-gray-800 dark:text-cream/80">${ssrInterpolate(c.content)}</p></li>`);
        });
        _push(`<!--]--></ul>`);
      }
      _push(`</section>`);
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(post)) {
        _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen bg-[#f5f3eb] dark:bg-[#0e0e0e]" }, _attrs))}><div class="relative h-[50vh] min-h-[360px] w-full overflow-hidden">`);
        if (unref(post).image) {
        ...
        _push(`<div class="absolute inset-0 bg-gradient-to-t from-[#f5f3eb] via-[#f5f3eb]/60 to-transparent dark:from-[#0e0e0e] dark:via-[#0e0e0e]/60"></div><div class="absolute inset-0 shadow-[inset_0_-120px_100px_-40px_rgba(0,0,0,0.9)]"></div><div class="absolute bottom-0 left-0 right-0 p-8 sm:p-12"><div class="mx-auto max-w-4xl">`);
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: "/",
          class: "mb-6 inline-flex items-center gap-2 text-sm font-medium text-gray-600 transition-colors hover:text-teal dark:text-cream/50 dark:hover:text-mint"
        }, {
        ...
        _push(`<h1 class="text-4xl font-extrabold leading-tight text-gray-900 dark:text-cream sm:text-6xl">${ssrInterpolate(unref(post).title)}</h1><div class="mt-4 flex flex-wrap items-center gap-4 text-gray-600 dark:text-cream/60"><span>${ssrInterpolate(unref(post).date)}</span><!--[-->`);
        ssrRenderList(unref(post).tags ?? [], (tag) => {
          _push(`<span class="rounded-full bg-gold/20 px-2 py-0.5 text-xs font-semibold text-gold">${ssrInterpolate(tag)}</span>`);
        });
        _push(`<!--]--></div></div></div></div><article class="mx-auto max-w-3xl px-8 py-16 sm:px-12"><div class="prose prose-gray-900 max-w-none dark:prose-invert dark:prose-cream">`);
        if (unref(post).body) {
...
//# sourceMappingURL=_slug_-DnM2i0yG.mjs.map
